...
for OPLOG in `ls $OPLOGS_DIRECTORY/*.bson.gz`; do
   ...
   if [ $OPLOG_TIMESTAMP -gt $FULL_DUMP_TIMESTAMP ]; then
      if [ $ALREADY_APPLIED_OPLOG -eq 0 ]; then
         ...
         mongorestore  --host="rs1/localhost:27019,localhost:27020" --oplogFile $LAST_OPLOG --oplogReplay --dir /tmp/emptyDirForOpRestore --oplogLimit=$OPLOG_LIMIT
         echo "applying oplog $OPLOG"
         mongorestore --host="rs1/localhost:27019,localhost:27020" --oplogFile $OPLOG --oplogReplay --dir /tmp/emptyDirForOpRestore --oplogLimit=$OPLOG_LIMIT
      else
         echo "applying oplog $OPLOG"
         mongorestore --host="rs1/localhost:27019,localhost:27020" --oplogFile $OPLOG --oplogReplay --dir /tmp/emptyDirForOpRestore --oplogLimit=$OPLOG_LIMIT
      fi
